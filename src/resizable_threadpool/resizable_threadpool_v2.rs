...
pub type ReplacePoolRule = Box<dyn Fn(usize, &str) -> TokioResult<Runtime> + Send + Sync>;
...
pub struct DaemonRuntimeHandle {
    inner: Weak<Mutex<DaemonRuntime>>,
}
...
pub struct ResizableRuntime {
    ...
    gc_runtime: DaemonRuntime,
    current_runtime: Arc<Mutex<DaemonRuntime>>,
    replace_pool_rule: ReplacePoolRule,
...
impl ResizableRuntime {
    pub fn new(
        ...
        ResizableRuntime {
            ...
            current_runtime: Arc::new(Mutex::new(DaemonRuntime {
    ...
    pub fn adjust_with(&mut self, new_size: usize) {
        ...
        info!(target: "Resizing thread pool", "thread_name {:?}, new_size {:?}", &thread_name, new_size);
